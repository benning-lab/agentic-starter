...
- **The hooks run from this folder, not from a copy.** `install.sh` writes their absolute paths
  into your settings, so a `git pull` here changes what runs on your machine. That is the
  convenience, and it is also the risk: treat a pull here the way you would treat any other code
  you execute.
